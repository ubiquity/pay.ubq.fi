...
`usePermitInvalidation` tracks `invalidationError`, but nothing renders it. The global `setError()` path *is* rendered already, so this is not strictly blocking, but it’s easy to improve clarity.
