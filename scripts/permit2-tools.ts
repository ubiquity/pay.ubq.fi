...
export const isHexAddress = (value: string) =>
...
export function createSupabaseClientFromEnv(options?: {
	...
	const key =
		preferServiceRole && serviceRoleKey
			? serviceRoleKey
	...
	return {
		...
		usesServiceRole: Boolean(key === serviceRoleKey),
		url,
	};
}
...
export async function fetchPermitsFromDb({
  ...
  `;
	...
	const buildQuery = () => {
		...
		if (ownerFilter)
			query = query.filter("partner.wallet.address", "ilike", ownerFilter);
...
export function noncePositions(nonce: bigint): {
	wordPos: bigint;
	bitPos: bigint;
} {
...
const permit2NonceBitmapAbiItem = parseAbiItem(
	"function nonceBitmap(address owner, uint256 wordPos) view returns (uint256)",
);

export function encodeNonceBitmapCall({
	owner,
	wordPos,
}: {
	owner: `0x${string}`;
	...
	const fetchChunk = async (
		...
		const requests: JsonRpcRequest[] = chunkRefs.map((ref, idx) => {
			...
			return {
				...
				params: [
					{
						to: ref.permit2Address,
						data: encodeNonceBitmapCall({
							owner: ref.owner,
							wordPos: ref.wordPos,
						}),
					},
		...
		try {
			responses = await rpcBatchCallWithRetries({
				rpcBaseUrl,
				chainId,
				requests,
				maxRetries,
			...
			if (chunkRefs.length <= 1) {
				for (const ref of chunkRefs)
					out.set(bitmapKey(ref), { error: message });
				return;
			}
		...
		for (const [id, key] of idToKey.entries()) {
			...
			try {
				...
				out.set(key, {
					error: error instanceof Error ? error.message : "Bitmap parse error",
				});
			}
		}
	};
