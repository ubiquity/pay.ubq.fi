...
const INVALIDS_URL = new URL(
	"../reports/permit2-backfill-invalids-retained.json",
	import.meta.url,
);
const REPORT_URL = new URL(
	"../reports/permit2-backfill-invalids-regenerated-status.json",
	import.meta.url,
);
...
const main = async () => {
	const invalids = JSON.parse(
		await Deno.readTextFile(INVALIDS_URL),
	) as InvalidRow[];
	const ids = invalids
		.map((row) => Number(row.id))
		.filter((id) => Number.isFinite(id));
	const nonces = invalids
		.map((row) => row.nonce)
	...
	const { data: byId, error: idError } = await supabase
		...
		.in("id", ids);
	if (idError)
		throw new Error(`Failed to fetch permits by id: ${idError.message}`);

	const { data: byNonce, error: nonceError } = await supabase
		.from("permits")
		.select(selectQuery)
		.in("nonce", nonces);
	if (nonceError)
		throw new Error(`Failed to fetch permits by nonce: ${nonceError.message}`);
	...
	const locationRows =
		nodeUrls.length > 0
			? await supabase
					.from("locations")
					.select("id,node_url")
					.in("node_url", nodeUrls)
			: { data: [] as { id: number; node_url?: string | null }[], error: null };
	if (locationRows.error)
		throw new Error(`Failed to fetch locations: ${locationRows.error.message}`);
	...
	if (locationError)
		throw new Error(
			`Failed to fetch permits by location: ${locationError.message}`,
		);
	...
	const report = invalids.map((entry) => {
		...
		const replacements = nonceRows.filter(
			(item) => item.id !== id && item.partner_id !== null,
		);
		const nodeUrl = entry.node_url ?? null;
		const nodeRows = nodeUrl ? (byNodeUrlMap.get(nodeUrl) ?? []) : [];
		const regenCandidates = nodeRows.filter(
			(item) => item.partner_id !== null && isValidSignature(item.signature),
		);
	...
	const summary = {
		...
		still_invalid: report.filter(
			(entry) =>
				entry.invalid_row.exists && entry.invalid_row.partner_id === null,
		).length,
		replacements_found: report.filter(
			(entry) => entry.nonce_matches.replacement_count > 0,
		).length,
		regenerated_by_node_url: report.filter(
			(entry) => entry.node_url_matches.regenerated_count > 0,
		).length,
	};
	...
	console.log(
		JSON.stringify({ summary, report_path: REPORT_URL.pathname }, null, 2),
	);
};
