...
const REPORT_URL = new URL(
	"../reports/permit2-backfill-remaining.json",
	import.meta.url,
);
const OUT_JSON_URL = new URL(
	"../reports/permit2-backfill-partner-not-found.json",
	import.meta.url,
);
const OUT_CSV_URL = new URL(
	"../reports/permit2-backfill-partner-not-found.csv",
	import.meta.url,
);
...
const main = async () => {
	...
	const partnerNotFound = skipped.filter(
		(entry) => entry.reason === "partnerNotFound",
	);
	...
	const { data, error } = await supabase
		.from("permits")
		.select(
			"id,amount,nonce,deadline,token_id,network_id,permit2_address,created,location:locations(node_url)",
		)
		.in("id", ids);
	...
	const rows = partnerNotFound.map((entry) => {
		...
		const ownerCandidates = Array.from(
			new Set(recoveries.map((rec) => rec.owner)),
		).join("|");
