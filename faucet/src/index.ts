...
export default {
  async fetch(
    ...
    if (!ethAddress) {
        ...
        { error: { code: -32000, message: "No address provided" } },
        400
      );
	  }
    ...
    const { data: addressInPermits } = await supabase
      .from('permits')
      .select('bounty_hunter_address')
      .eq('bounty_hunter_address', ethAddress);
    ...
    if (relayBal.result < claimFee) {
      return makeRpcResponse(
        { error: { code: -32000, message: "Faucet has no funds" } },
        400
      );
    }
