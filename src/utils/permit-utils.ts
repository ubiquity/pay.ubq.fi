...
export const formatAmount = (
	...
	try {
		...
		if (isNaN(numericValue)) {
			console.warn(
				`formatAmount: formatted value "${formatted}" resulted in NaN.`,
			);
			return Number(0).toFixed(displayDecimals);
		}
		...
		console.warn(
			`Amount formatting failed for amount: ${rawAmount}, decimals: ${decimals}`,
			error,
		);
		return Number(0).toFixed(displayDecimals);
	}
...
export const hasRequiredFields = (permit: PermitData): boolean => {
	...
	if (permit.type === "erc20-permit") {
		...
		if (!permit.tokenAddress && !permit.token?.address)
			errors.push("token address (for ERC721)");
		if (permit.token_id === undefined || permit.token_id === null)
			errors.push("token_id (for ERC721)");
	} else {
...
export async function queuePermitClaims(
	...
	writeContractAsync: (
		permit: PermitData,
		options: { mode: "recklesslyUnprepared" },
	) => Promise<unknown>,
) {
