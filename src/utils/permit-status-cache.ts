...
export function loadPermitStatusCache(
	...
	try {
		...
		for (const [signature, entry] of Object.entries(
...
export function savePermitStatusCache(
	walletAddress: string,
	cache: PermitStatusCache,
): void {
	if (typeof localStorage === "undefined") return;
	try {
...
export function upsertPermitStatusOverride(
	...
	cache[normalizedSignature] = {
		...
		isNonceUsed:
			patch.isNonceUsed !== undefined
				? patch.isNonceUsed
				: previous?.isNonceUsed,
		transactionHash:
...
export function applyPermitStatusOverrides(
	permit: PermitData,
	cache: PermitStatusCache,
): PermitData {
	...
	return {
		...
		...(override.isNonceUsed !== undefined && {
			isNonceUsed: override.isNonceUsed,
		}),
		...(override.transactionHash !== undefined && {
			transactionHash: override.transactionHash,
		}),
	};
}
