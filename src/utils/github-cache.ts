...
class GitHubUsernameCache {
		...
		try {
			...
			if (raw) {
				...
				const resetAtMs =
					...
					Number.isFinite(parsed.resetAtMs)
						? parsed.resetAtMs
						: null;
				const limitedUntilMs =
					typeof parsed.limitedUntilMs === "number" &&
					Number.isFinite(parsed.limitedUntilMs)
						? parsed.limitedUntilMs
						: null;
	...
	private markRateLimited(
		...
		if (typeof resetAtMs === "number" && Number.isFinite(resetAtMs)) {
			this.rateLimit.resetAtMs = Math.max(
				this.rateLimit.resetAtMs ?? 0,
				resetAtMs,
			);
		}
		this.persistRateLimit();
	...
	private updateRateLimitFromHeaders(headers: Headers): void {
		...
		if (
			typeof resetUnixSeconds === "number" &&
			Number.isFinite(resetUnixSeconds)
		) {
		...
		const promise = (async () => {
			try {
				...
				if (!response.ok) {
					...
					console.warn(
						`Failed to fetch GitHub user ${userId}: ${response.status}`,
					);
					return null;
				}
				...
				console.error(
					`Error fetching GitHub username for user ${userId}:`,
					error,
				);
				return null;
			} finally {
