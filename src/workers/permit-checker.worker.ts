...
worker.onmessage = async (event) => {
	...
	if (type === "INIT") {
		...
		if (supabaseUrl && supabaseAnonKey) {
			...
			worker.postMessage({
				type: "INIT_ERROR",
				error: "Supabase/RPC credentials not received by worker.",
			});
		}
    ...
    `;
			...
			for (let offset = 0; ; offset += pageSize) {
				...
				if (result.error) {
					console.error(
						`Worker: owner query error: ${result.error.message}`,
						result.error,
					);
			...
			beneficiaryPermitsFromDb.forEach((permit) =>
			...
			if (ownerPermitsFromDb.length > 0) {
				console.log(
					`Worker: Found ${ownerPermitsFromDb.length} permits as owner`,
				);
			}
				...
				await Promise.all(
					Array.from(permitMap.values()).map(async (permit, index) => {
						...
						if (!beneficiaryWalletAddress) {
							console.warn(
								`Worker: Permit ${permit.id} missing beneficiary wallet address; skipping`,
							);
							return null;
						}
						...
						const mapped = await mapDbPermitToPermitData({
							permit,
							index,
							lowerCaseWalletAddress: beneficiaryAddress,
						});
			...
			if (mappedNewPermits.length > 0) {
				const validatedNewPermits = await validatePermitsBatch({
					rpcClient: rpc,
					permitsToValidate: mappedNewPermits,
				});
				worker.postMessage({
					type: "NEW_PERMITS_VALIDATED",
					requestId,
