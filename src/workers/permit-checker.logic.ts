...
import type {
	JsonRpcResponse,
	createRpcClient,
} from "@ubiquity-dao/permit2-rpc-client";
import {
	type Address,
	encodeFunctionData,
	erc20Abi,
...
export async function mapDbPermitToPermitDataWithIssues({
	...
	if (permit.partner && !permit.partner.wallet)
		issues.push("missing_partner_wallet");

	const ownerWalletData = permit.partner?.wallet;
	const ownerAddressStr = ownerWalletData?.address
		? String(ownerWalletData.address)
		: "";
	...
	const tokenAddressStr = tokenData?.address
	...
	if (!Number.isFinite(networkIdNum) || networkIdNum <= 0)
		issues.push("invalid_token_network");

	if (!permit.deadline) {
		issues.push("missing_deadline");
	} else if (
		typeof permit.deadline !== "string" ||
		isNaN(parseInt(permit.deadline, 10))
	) {
	...
	if (!permit.signature) {
		...
		!isHexString(permit.signature) ||
		(permit.signature.length !== 130 && permit.signature.length !== 132)
	) {
	...
	const githubUrlStr = permit.location?.node_url
		? String(permit.location.node_url)
		: "";
	...
	const permitData: PermitData = {
		...
		owner: ownerAddressStr,
		tokenAddress: tokenAddressStr,
		token: tokenAddressStr
			? { address: tokenAddressStr, network: networkIdNum }
			: undefined,
		amount: BigInt(permit.amount),
		token_id:
			permit.token_id !== undefined && permit.token_id !== null
				? Number(permit.token_id)
				: undefined,
		githubCommentUrl: githubUrlStr,
		partner: ownerAddressStr
			? { wallet: { address: ownerAddressStr } }
			: undefined,
		claimStatus: "Idle",
		status: "Fetching",
...
export async function mapDbPermitToPermitData({
	...
	const { permitData, issues } = await mapDbPermitToPermitDataWithIssues({
		permit,
		lowerCaseWalletAddress,
	});
	...
	if (issues.includes("missing_partner_wallet_address")) {
		logger.warn(
			`PermitChecker: Permit [${index}] with nonce ${permit.nonce} has no owner address`,
		);
	} else if (issues.includes("missing_token_address")) {
		logger.warn(
			`PermitChecker: Permit [${index}] with nonce ${permit.nonce} has no token address`,
		);
	} else if (issues.includes("invalid_token_network")) {
		logger.warn(
			`PermitChecker: Permit [${index}] with nonce ${permit.nonce} has invalid network ID: ${permit.token?.network}`,
		);
	} else if (
		issues.includes("missing_deadline") ||
		issues.includes("invalid_deadline")
	) {
		logger.warn(
			`PermitChecker: Permit [${index}] with nonce ${permit.nonce} has invalid deadline: ${permit.deadline}`,
		);
	} else if (
		issues.includes("missing_signature") ||
		issues.includes("invalid_signature_prefix") ||
		issues.includes("invalid_signature_format")
	) {
		logger.warn(
			`PermitChecker: Permit [${index}] with nonce ${permit.nonce} has invalid signature format: ${permit.signature}`,
		);
	} else if (issues.includes("invalid_amount")) {
		logger.warn(
			`PermitChecker: Permit [${index}] with nonce ${permit.nonce} has invalid amount format: ${permit.amount}`,
		);
	} else if (issues.includes("signature_recovery_failed")) {
		logger.warn(
			`PermitChecker: Permit [${index}] with nonce ${permit.nonce} signature recovery failed`,
		);
	} else {
		logger.warn(
			`PermitChecker: Permit [${index}] with nonce ${permit.nonce} skipped: ${issues.join(", ")}`,
		);
	}
...
export async function fetchPermitsFromDb({
	...
	for (let offset = 0; ; offset += pageSize) {
		...
		if (result.error) {
			logger.error(
				`PermitChecker: query error: ${result.error.message}`,
				result.error,
			);
			return [];
		}
...
async function getPermit2Address(permitData: {
	...
	const hash = hashTypedData({
		domain: {
			...
			verifyingContract: NEW_PERMIT2_ADDRESS,
		},
...
export async function validatePermitsBatch({
	...
	if (permitsToValidate.length === 0) {
		return {
			permits: [],
			balancesAndAllowances: new Map<string, AllowanceAndBalance>(),
		};
	}

	const checkedPermitsMap = new Map<
		string,
		Partial<PermitData & { isNonceUsed?: boolean }>
	>();
	const balancesAndAllowances = new Map<string, AllowanceAndBalance>();
	const permitsByKey = new Map<string, PermitData>(
		permitsToValidate.map((p) => [p.signature, p]),
	);
	...
	for (const [networkId, permits] of permitsByNetwork.entries()) {
		...
		permits.forEach((permit) => {
			...
			if (permit.token?.address && permit.amount && permit.owner) {
				...
				if (!balancesAndAllowances.has(balanceKey)) {
					...
					balancesAndAllowances.set(balanceKey, {
						networkId,
						permit2Address: permit.permit2Address,
						tokenAddress: permit.token.address,
						owner,
					});
				}
			}
		...
		try {
			...
			const batchResponses = (await rpcClient.request(
				networkId,
				batchPayload,
			)) as JsonRpcResponse[];
			const responseMap = new Map<number, JsonRpcResponse>(
				batchResponses.map((res) => [res.id as number, res]),
			);
			...
			batchRequests.forEach((batchReq) => {
				...
				if (batchReq.type === "nonce") {
					...
					const updateData: Partial<PermitData & { isNonceUsed?: boolean }> =
						checkedPermitsMap.get(batchReq.key) || {};
					...
					if (!res) {
						...
						try {
							...
							updateData.isNonceUsed = Boolean(
								bitmap & (1n << (BigInt(permit.nonce) & 255n)),
							);
					...
					batchReq.type === "balance" ||
					batchReq.type === "allowance"
				) {
					...
					if (
						balanceAllowanceData.balance !== undefined &&
						balanceAllowanceData.allowance !== undefined &&
						!balanceAllowanceData.error
					) {
						balanceAllowanceData.maxClaimable =
							balanceAllowanceData.balance < balanceAllowanceData.allowance
								? balanceAllowanceData.balance
								: balanceAllowanceData.allowance;
					}
			...
			permits.forEach((permit) => {
				...
				if (balanceAllowanceData) {
					const updateData: Partial<PermitData & { isNonceUsed?: boolean }> =
						checkedPermitsMap.get(permit.signature) || {};

					if (balanceAllowanceData.error) {
						updateData.checkError = balanceAllowanceData.error;
					} else {
						if (balanceAllowanceData.balance !== undefined) {
							updateData.ownerBalanceSufficient =
								balanceAllowanceData.balance >= permit.amount;
						}
						if (balanceAllowanceData.allowance !== undefined) {
							updateData.permit2AllowanceSufficient =
								balanceAllowanceData.allowance >= permit.amount;
						}
					}
			...
			logger.error(
				"PermitChecker: Error during validation batch RPC request:",
				error,
			);
