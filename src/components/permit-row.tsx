...
export function PermitRow({
	...
	const isOwner =
		!!address && permit.owner.toLowerCase() === address.toLowerCase();
	...
	const rowClassName = (() => {
		...
		if (insufficientBalance || insufficientAllowance || prerequisiteCheckFailed)
			return "row-invalid";
	...
	const targetNetworkName =
		NETWORK_NAMES[permit.networkId] || `Network ${permit.networkId}`;
	const canSwitchToPermitNetwork = switchableChains.some(
		(c: Chain) => c.id === permit.networkId,
	);
	const explorerUrl = chain?.blockExplorers?.default?.url;

	const statusDisplayText = (() => {
		if (networkMismatch)
			return `Switch wallet to ${targetNetworkName} to ${isFundingWallet && isOwner ? "invalidate" : "claim"}`;
	...
	const showCannotClaimIcon =
		!networkMismatch && !canAttemptClaim && !isClaimed && !isClaimingThis;
			...
			if (parsed) {
				return (
					<>
						...
						{isFundingWallet && (
							<span
								title={`Beneficiary wallet: ${permit.beneficiary}`}
								style={{ cursor: "help" }}
								className="github-beneficiary"
							>
								{githubUsername
									? githubUsername
									: truncateAddress(permit.beneficiary)}
							</span>
						)}
					</>
				);
	...
	const renderAmount = () => {
		...
		if (permit.quoteError) {
			return (
				<span title={`Quote Error: ${permit.quoteError}`}>
					{ICONS.WARNING} Error
				</span>
			);
		}
		...
		if (permit.estimatedAmountOut && preferredRewardTokenAddress) {
			...
			if (preferredTokenInfo) {
				try {
					const estimatedValueString = formatUnits(
						BigInt(permit.estimatedAmountOut),
						preferredTokenInfo.decimals,
					);
					...
					const originalTokenInfo = getTokenInfo(
						chain?.id,
						permit.tokenAddress as Address,
					);
					const originalSymbol = originalTokenInfo?.symbol || "tokens";
					const originalAmountFormatted =
						permit.amount && originalTokenInfo
							? formatAmount(permit.amount, originalTokenInfo.decimals)
							: "N/A";
					...
					if (explorerUrl && tokenAddress && ownerAddress) {
						return (
							<button
								...
								onClick={() =>
									window.open(
										`${explorerUrl}/token/${tokenAddress}?a=${ownerAddress}`,
										"_blank",
									)
								}
						...
						return (
							<span
								className="monospace"
								title={`Original: ${originalAmountFormatted} ${originalSymbol}`}
							>
								≈ {displayValue} {preferredTokenInfo.symbol}
							</span>
						);
					...
					return (
						<span title="Error formatting estimated amount">
							{ICONS.WARNING} Format Error
						</span>
					);
				}
			}
		...
		if (permit.type === "erc20-permit" && permit.amount) {
			...
			if (explorerUrl && permit.owner && permit.tokenAddress) {
				...
				return (
					<span
						title={`Token: ${originalSymbol} (${permit.tokenAddress}) on ${targetNetworkName}`}
					>
						{originalTokenInfo
							? formatAmount(permit.amount, originalTokenInfo.decimals)
							: "N/A"}{" "}
	...
	return (
		<div className={`permit-row ${rowClassName}`}>
			...
			<div className="permit-cell actions-cell">
				...
				{!networkMismatch && permit.claimError && (
