...
function isUserRejectedRequest(error: unknown): boolean {
	...
	if (maybeAny && typeof maybeAny === "object") {
		...
		if (
			typeof maybeAny.name === "string" &&
			maybeAny.name.toLowerCase().includes("userrejected")
		)
			return true;
	}
	...
	return /user rejected|user denied|rejected the request|denied transaction signature|request rejected|action_rejected/i.test(
		message,
	);
}
...
export function DashboardPage() {
	...
	const [preferredRewardTokenAddress, setPreferredRewardTokenAddress] =
	...
	const claimablePermits = useMemo(() => {
		...
		const availableClaimAmount = new Map(
			Array.from(balancesAndAllowances.entries()).map(([key, value]) => [
				key,
				value.maxClaimable,
			]),
		);
	...
	const invalidatablePermits = useMemo(() => {
		...
		return permits.filter(
			(p) =>
				p.owner.toLowerCase() === normalizedAddress &&
				p.status !== "Claimed" &&
				p.isNonceUsed !== true,
		);
	}, [permits, address]);
		...
		for (const permit of claimablePermits) {
			if (permit.amount) {
				try {
					...
					console.error(
						`Error parsing amount for claimableTotalValue calc: ${permit.amount}`,
						e,
					);
				}
			}
	...
	const estimatedTotalValueDisplay = useMemo(() => {
		...
		const preferredTokenInfo = getTokenInfo(
			chain?.id,
			preferredRewardTokenAddress,
		);
		...
		const permitsToConsider = permits.filter((p) =>
			claimablePermits.some(
				(cp) => cp.nonce === p.nonce && cp.networkId === p.networkId,
			),
		);

		permitsToConsider.forEach((permit) => {
			if (
				permit.tokenAddress?.toLowerCase() ===
				preferredRewardTokenAddress.toLowerCase()
			) {
				if (permit.amount) {
					try {
						...
						console.error(
							`Error parsing original amount for estimatedTotalValue calc: ${permit.amount}`,
							e,
						);
					}
				}
				...
				try {
					...
					console.error(
						`Error parsing estimated amount for estimatedTotalValue calc: ${permit.estimatedAmountOut}`,
						e,
					);
				}
			}
		...
		try {
			const formattedValue = parseFloat(
				formatUnits(totalEstimatedValueInWei, preferredTokenInfo.decimals),
			);
	...
	const isInvalidatingAny = useMemo(
		() => Object.values(isInvalidating).some(Boolean),
		[isInvalidating],
	);
	...
	const isClaimFlowActive = useMemo(
		() =>
			...
			(pendingNetworkSwitch.isSwitching &&
				pendingNetworkSwitch.action === "claim"),
	...
	const isInvalidationFlowActive = useMemo(
		() =>
			isInvalidatingAny ||
			(pendingNetworkSwitch.isSwitching &&
				pendingNetworkSwitch.action === "invalidate"),
		[
			isInvalidatingAny,
			pendingNetworkSwitch.isSwitching,
			pendingNetworkSwitch.action,
		],
	);
	...
	const claimPermits = useCallback(
		async (permitsToClaim: PermitData[]) => {
			if (!isConnected || !address || !chain) {
				console.error(
					"Cannot claim permits: Wallet not connected or address/chain missing",
				);
				return;
			}
			...
			const networksToClaim = [
				currentNetworkId,
				...Array.from(permitsByNetwork.keys()).filter(
					(id) => id !== currentNetworkId,
				),
			];
			...
			for (const networkId of networksToClaim) {
				...
				try {
					...
					const batchablePermits = permitsForNetwork.filter(
						(p) =>
							p.permit2Address.toLowerCase() ===
							NEW_PERMIT2_ADDRESS.toLowerCase(),
					);
					const sequentialPermits = permitsForNetwork.filter(
						(p) =>
							p.permit2Address.toLowerCase() ===
					...
					if (batchablePermits.length > 0) {
						console.log(
							`Claiming ${batchablePermits.length} batchable permits on network ${networkId}`,
						);
					...
					if (sequentialPermits.length > 0) {
						console.log(
							`Claiming ${sequentialPermits.length} sequential permits on network ${networkId}`,
						);
						await handleClaimSequential(sequentialPermits);
					}
				} catch (error) {
					console.error(
						`Error claiming permits on network ${networkId}:`,
						error,
					);
				}
			}
	...
	const invalidatePermits = useCallback(
		async (permitsToInvalidate: PermitData[]) => {
			if (!isConnected || !address || !chain) {
				console.error(
					"Cannot invalidate permits: Wallet not connected or address/chain missing",
				);
				return;
			}
			const currentNetworkId = chain.id;
			const normalizedAddress = address.toLowerCase();
			const ownedPermits = permitsToInvalidate.filter(
				(p) => p.owner.toLowerCase() === normalizedAddress,
			);
				...
				try {
					...
					if (lastHash) {
						setLastTx({
							txHash: lastHash,
							chainId: networkId,
							label: "Permits invalidated",
						});
					}
					if (!res.success) return;
				} catch (error) {
					console.error(
						`Error invalidating permits on network ${networkId}:`,
						error,
					);
					return;
				}
	...
	useEffect(() => {
		if (
			...
			const signatureSet = new Set(
				pendingNetworkSwitch.permits.map((p) => p.signature.toLowerCase()),
			);

			const permitsToResume =
				action === "invalidate"
					? invalidatablePermits.filter((p) =>
							signatureSet.has(p.signature.toLowerCase()),
						)
					: claimablePermits.filter((p) =>
							signatureSet.has(p.signature.toLowerCase()),
						);
			...
			if (action === "invalidate") {
				void invalidatePermits(permitsToResume).catch((error) => {
					console.error(
						"Failed to resume invalidation flow after network switch:",
						error,
					);
				});
				return;
			...
			void claimPermits(permitsToResume).catch((error) => {
				console.error(
					"Failed to resume claim flow after network switch:",
					error,
				);
			});
		}
				...
				{isConnected && address ? (
					<>
						...
						<button
							...
							onClick={() => {
								if (isFundingWallet) {
									void invalidatePermits(invalidatablePermits).catch(
										(error) => {
											console.error(
												"Failed to start invalidation flow:",
												error,
											);
										},
									);
								...
								if (claimablePermitCount > 0) {
									setClaimAllSnapshot({
										amountDisplay: estimatedTotalValueDisplay,
										rewardCount: claimablePermitCount,
									});
								}
							...
							title={
								isFundingWallet
									? "Invalidate all valid permits (batched by nonce bitmap)"
									: "Claim all valid and available permits (batch RPC)"
							}
						>
							{isFundingWallet ? (
								isInvalidationFlowActive ? (
										...
										{isFundingWallet ? (
											<>
												...
												<span className="claim-count">
													({invalidatablePermitCount} Permit
													{invalidatablePermitCount !== 1 ? "s" : ""})
												</span>
											</>
										) : isClaimFlowActive ? (
											<>
												<span className="claim-amount">
													{claimAllSnapshot?.amountDisplay ??
														estimatedTotalValueDisplay}
												</span>
