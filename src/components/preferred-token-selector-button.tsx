...
import {
	getSupportedRewardTokensForChain,
	getTokenInfo,
} from "../constants/supported-reward-tokens";
...
export function PreferredTokenSelectorButton({
	chainId,
	onPreferenceChange,
}: PreferredTokenSelectorButtonProps) {
	const [isDropdownVisible, setIsDropdownVisible] = useState(false);
	const [currentPreference, setCurrentPreference] = useState<Address | null>(
		null,
	);
	...
	useEffect(() => {
		...
		if (
			...
			availableTokens.some(
				(token) =>
					token.address.toLowerCase() === storedPreference.toLowerCase(),
			)
		) {
	...
	return (
		<div className="preferred-token-selector-button-wrapper" ref={wrapperRef}>
			<button
				...
				className="preferred-token-button"
				title="Select Preferred Reward Token"
			>
				{displayIcon}
			</button>
			{isDropdownVisible && (
				<div className="preference-dropdown-container">
					<RewardPreferenceSelector
						chainId={chainId}
						onPreferenceChange={handleInternalPreferenceChange}
					/>
				</div>
			)}
		</div>
