...
- Start the backend (`bun run src/backend/server.ts`)
...
This command runs `tsc -b` for type checking and then `vite build`. The output files will be generated in the `src/frontend/dist` directory.
...
From the `src/frontend` directory, run the deploy script:
...
3.  Deploying the contents of the `dist` directory using `deployctl`.
    *   It uses `serve.ts` as the entry point to serve static files and handle SPA routing.
