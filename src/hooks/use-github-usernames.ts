...
export function useGithubUsernames(permits: PermitData[]) {
	...
	const retryStateRef = useRef(
		new Map<number, { attempt: number; nextAttemptAtMs: number }>(),
	);
	...
	const beneficiaryUserIdsKey = useMemo(() => {
		...
		for (const permit of permits) {
			if (
				typeof permit.beneficiaryUserId === "number" &&
				permit.beneficiaryUserId > 0
			) {
	...
	useEffect(() => {
		...
		const scheduleRetry = (userId: number) => {
			...
			const delay = Math.min(
				BASE_RETRY_DELAY_MS * 2 ** Math.min(attempt - 1, 6),
				MAX_RETRY_DELAY_MS,
			);
			retryStateRef.current.set(userId, {
				attempt,
			...
			try {
				...
				while (!cancelled) {
					...
					const eligibleUserIds = unresolvedUserIds.filter((id) => {
						const nextAtMs =
							retryStateRef.current.get(id)?.nextAttemptAtMs ?? 0;
						return nextAtMs <= now;
					});

					if (eligibleUserIds.length === 0) {
						const nextAttemptAtMs = Math.min(
							...unresolvedUserIds.map(
								(id) =>
									retryStateRef.current.get(id)?.nextAttemptAtMs ??
									now + 60_000,
							),
						);
					...
					const maxBatchSize =
						remaining == null
							? BATCH_SIZE
							: Math.max(0, Math.min(BATCH_SIZE, remaining));
