...
interface UsePermitClaimingProps {
	...
	updatePermitStatusCache: (
		permitKey: string,
		status: Partial<PermitData>,
	) => void;
	publicClient: PublicClient | null;
	walletClient: WalletClient | null;
	address: Address | undefined;
	chain: Chain | null;
	setBalancesAndAllowances: Dispatch<
		SetStateAction<Map<string, AllowanceAndBalance>>
	>;
}

async function simulatePermitTranferFrom(
...
async function simulateBatchPermitTransferFrom(
	publicClient: PublicClient,
	address: Address,
...
> {
	...
	for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
		...
		if (!res.retryable)
			return { ok: false, attempts: attempt, lastError: res.error };
		await sleep(delayMs);
	...
	return {
		...
		lastError: "record-claim retries exhausted",
	};
}
...
export function usePermitClaiming({
	...
	const [sequentialClaimError, setSequentialClaimError] = useState<
		string | null
	>(null);
	const [swapSubmissionStatus] = useState<
				...
				if (existing) {
					newMap.set(key, {
						...
						allowance: existing.allowance
							? existing.allowance - permit.amount
							: undefined,
						balance: existing.balance
							? existing.balance - permit.amount
							: undefined,
						maxClaimable: existing.maxClaimable
							? existing.maxClaimable - permit.amount
							: undefined,
					});
				}
			}
	...
	const handleClaimPermit = async (
		...
		setPermits((prev) =>
			prev.map((p) =>
				p.signature === permit.signature ? { ...p, claimStatus: "Pending" } : p,
			),
		);
		...
		try {
			...
			const { request } = await simulatePermitTranferFrom(
				publicClient,
				address,
				permit,
			);
			...
			setPermits((prev) =>
				prev.map((p) =>
					p.signature === permit.signature
						? { ...p, claimStatus: "Pending", transactionHash: txHash }
						: p,
				),
			);
			updatePermitStatusCache(permit.signature, {
				claimStatus: "Pending",
			...
			try {
				...
				console.warn(
					"Receipt lookup failed after tx submission; will attempt to record via API",
					{ error, txHash, networkId: permit.networkId },
				);
			}

			if (!receipt) {
				setError(
					`Claim tx submitted but confirmation failed. Check explorer: ${txHash}`,
				);
				void recordClaimWithRetries({
					txHash,
					networkId: permit.networkId,
				}).then((result) => {
					if (!result.ok) {
						console.warn("record-claim did not succeed after retries", {
							txHash,
							networkId: permit.networkId,
							...result,
						});
			...
			void recordClaimWithRetries({ txHash, networkId: permit.networkId }).then(
				(result) => {
					if (!result.ok)
						console.warn("Failed to record claim after retries", {
							txHash,
							networkId: permit.networkId,
							...result,
						});
			...
			if (txHash) {
				if (errorMessage.includes("Transaction failed with status")) {
					setPermits((prev) =>
						prev.map((p) =>
							p.signature === permit.signature
								? { ...p, claimStatus: "Error", transactionHash: txHash }
								: p,
						),
					);
					return { success: false, txHash };
	...
	const handleClaimSequential = async (permitsToClaim: PermitData[]) => {
		if (!walletClient || !address || !chain || !publicClient) {
			console.error(
				"Sequential claim: Wallet not connected - client:",
				walletClient,
				"address:",
				address,
				"chain:",
			...
			console.log(
				"Sequential claim: Wallet connection verified - address:",
				address,
				"chain id:",
				chain.id,
			);
		...
		setPermits((prev) =>
			prev.map((p) =>
				toClaim.some((c) => c.signature === p.signature)
	...
	const handleClaimBatch = async (permitsToClaim: PermitData[]) => {
		if (!walletClient || !address || !chain || !publicClient) {
			console.error(
				"Batch RPC: Wallet not connected - client:",
				walletClient,
				"address:",
				address,
				"chain:",
			...
			console.log(
				"Batch RPC: Wallet connection verified - address:",
				address,
				"chain id:",
				chain.id,
			);
		...
		const networkIds = new Set(
			permitsToClaim.map((permit) => permit.networkId),
		);
		const batchNetworkId =
			networkIds.size === 1 ? permitsToClaim[0].networkId : null;
		try {
			// Update all permits to pending status
			setPermits((prev) =>
				prev.map((p) =>
			...
			const { request } = await simulateBatchPermitTransferFrom(
				publicClient,
				address,
				permitsToClaim,
			);
			...
			try {
				...
				console.warn(
					"Receipt lookup failed after tx submission; will attempt to record via API",
					{ error, txHash, networkId: batchNetworkId },
				);
			}
			...
			if (!receipt) {
				if (!batchNetworkId) {
					console.error(
						"Batch claim expects all permits to share the same networkId",
					);
					setError(
						"Batch claim submitted but could not be recorded (network mismatch).",
					);
				} else {
					setError(
						`Batch claim tx submitted but confirmation failed. Check explorer: ${txHash}`,
					);
					void recordClaimWithRetries({
						txHash,
						...
						if (!result.ok) {
							console.warn("Failed to record batch claim after retries", {
								txHash,
								networkId: batchNetworkId,
								...result,
							});
			...
			if (!batchNetworkId) {
				...
				void recordClaimWithRetries({ txHash, networkId: batchNetworkId }).then(
					(result) => {
						if (!result.ok)
							console.warn("Failed to record batch claim after retries", {
								txHash,
