...
export function usePermitInvalidation({
	...
	const handleInvalidatePermitsBatch = useCallback(
		async (
			...
			const notOwned = permits.filter(
				(permit) => permit.owner.toLowerCase() !== normalizedAddress,
			);
			...
			for (const group of grouped.values()) {
				...
				try {
					...
					if (receipt.status !== "success") {
						throw new Error(
							`Invalidation failed with status: ${receipt.status}`,
						);
					}
					...
					setPermits((prev) =>
						prev.map((p) =>
							groupPermitKeys.includes(p.signature)
								? {
										...p,
										status: "Claimed",
										isNonceUsed: true,
