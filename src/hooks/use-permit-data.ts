...
import {
	applyPermitStatusOverrides,
	loadPermitStatusCache,
	upsertPermitStatusOverride,
} from "../utils/permit-status-cache.ts";
import type { WorkerResponse } from "../workers/permit-checker.worker.ts";
import {
	getPermitCheckerWorker,
	type PermitCheckerWorker,
} from "../workers/permit-worker-client.ts";
...
export function usePermitData({
	...
	const [balancesAndAllowances, setBalancesAndAllowances] = useState<
		Map<string, AllowanceAndBalance>
	>(new Map());
	...
	const filterPermits = useCallback(
		(permitsMap: Map<string, PermitData>) => {
			...
			permitsMap.forEach((permit) => {
				const nonceCheckFailed = !!(
					permit.checkError && permit.checkError.toLowerCase().includes("nonce")
				);
				const shouldFilter =
	...
	const fetchQuotes = useCallback(
		async (
			...
			for (const [tokenIn, group] of byToken.entries()) {
				if (
					tokenIn.toLowerCase() === preferredRewardTokenAddress.toLowerCase()
				) {
					group.forEach((p) => {
				...
				group.forEach((p) => {
					if (p.amount) {
						try {
							...
							console.warn("Failed to parse permit amount", {
								amount: p.amount,
								error: e,
							});
						}
	...
	useEffect(() => {
		...
		const initWorker = async () => {
			try {
				...
				console.warn(
					"[use-permit-data] Worker initialization failed:",
					message,
				);
				setError(message);
				setWorker(null);
			...
			switch (data.type) {
				case "NEW_PERMITS_VALIDATED": {
					...
					validated.forEach((permit) => {
						newPermits.set(
							permit.signature,
							applyPermitStatusOverrides(permit, statusOverrides),
						);
					});
					...
					fetchQuotes(newPermits)
						...
						.catch((e) => {
							setError(
								`Failed to fetch swap quotes: ${e instanceof Error ? e.message : e}`,
							);
							setIsLoading(false);
						});
					break;
	...
	useEffect(() => {
		if (isConnected && address && chainId && worker && !isLoading) {
			fetchQuotes(new Map(allPermitsRef.current))
				...
				.catch((e: unknown) => {
					setError(
						`Failed to update swap quotes: ${e instanceof Error ? e.message : String(e)}`,
					);
